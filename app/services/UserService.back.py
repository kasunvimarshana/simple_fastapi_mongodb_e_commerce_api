...
class UserService:
    ...
    #             # =======================================
    #             # Fetch the most bought product and its quantity for each user
    #             for user_schema in user_schema_list:
    #                 most_bought_product = await OrderModel.aggregate([
    #                     {"": {"user": PydanticObjectId(user_schema.id)}},  # Match orders by user id
    #                     # {"$unwind": "$order_items"},  # Unwind order items
    #                     # {"$group": {"_id": "$order_items.product", "total_quantity": {"$sum": "$order_items.qty"}}},
    #                     # {"$sort": {"total_quantity": -1}},
    #                     # {"$limit": 1}
    #                 ]).to_list()

    #                 print("=====================================")
    #                 print("most_bought_product", most_bought_product)
    #                 print("=====================================")
    #             # =======================================
    ...
    #         try:
    #             query = UserModel.find(fetch_links=True)
    #             user_read_request_schema_dict = user_read_request_schema.model_dump(exclude_unset=True)
    ...
    #             # Aggregate to find users with their most purchased product
    #             query = query.aggregate(
    #                 [
    #                     {"$lookup": {"from": "orders", "localField": "_id", "foreignField": "user.$id", "as": "user_orders"}},
    #                     {"$unwind": "$user_orders"},
    #                     {"$lookup": {"from": "order_items", "localField": "user_orders._id", "foreignField": "order.$id", "as": "order_items"}},
    #                     {"$unwind": "$order_items"},
    #                     {"$group": {
    #                         "_id": "$_id",
    #                         "user": {"$first": "$$ROOT"},
    #                         "most_purchased_product": {"$max": "$order_items.qty"},
    #                     }},
    #                     {"$project": {
    #                         "user": 1,
    #                         "most_purchased_product": 1,
    #                     }},
    #                     {"$sort": {"user._id": pymongo.DESCENDING}}
    #                 ]
    #             )

    #             # Execute the query
    #             results = await query.to_list(length=user_read_request_schema_dict.get("limit", 0))

    #             print("results === ", results)

    #             # Build UserSchema objects from query results
    #             # user_schema_list = [UserSchema.model_validate(result["user"].model_dump(by_alias=True)) for result in results]

    #             # return PaginateResponseSchema[List[UserSchema]](count=len(user_schema_list), result=user_schema_list)
    #             return PaginateResponseSchema[List[UserSchema]](count=0, result=[])
    ...
    #             # Perform aggregation to retrieve users with most purchased product and sum of quantities
    #             pipeline = [
    #                 {"$match": query.raw_query},
    #                 {"$lookup": {
    #                     "from": "orders",
    #                     "localField": "_id",
    ...
    #                     "_id": "$_id",
    #                     "user": {"$first": "$$ROOT"},
    #                     "most_purchased_product": {"$first": "$order_items.product"},
    ...
    #             results = await UserModel.aggregate(pipeline).to_list(None)

    #             # Convert results to UserSchema objects
    #             user_schema_list = []
    #             for result in results:
    #                 user_data = result["user"].model_dump(by_alias=True)
    #                 user_schema = UserSchema.model_validate(user_data)
    #                 # user_schema.most_purchased_product = result["most_purchased_product"]
    #                 # user_schema.total_quantity = result["total_quantity"]
    ...
    async def read_users_with_most_sold_product(
            ...
            try:
                ...
                first_name_filter = user_read_request_schema_dict.get("first_name")
                if first_name_filter:
                    query_filters["first_name"] = first_name_filter

                # Filter by last_name if provided
                last_name_filter = user_read_request_schema_dict.get("last_name")
                if last_name_filter:
                    query_filters["last_name"] = last_name_filter

                # Filter by email if provided
                email_filter = user_read_request_schema_dict.get("email")
                if email_filter:
                    query_filters["email"] = email_filter

                # Filter by user_role if provided
                user_role_filter = user_read_request_schema_dict.get("user_role")
                if user_role_filter:
                    query_filters["user_role"] = user_role_filter
                ...
                # Match stage based on query filters
                if query_filters:
                    aggregation_pipeline.append({"$match": query_filters})

                # Lookup orders and order items
                ...
                # Get MongoDB collection for UserModel
                collection = db[UserModel.Settings.name]

                # Execute aggregation pipeline
                results = await UserModel.aggregate(aggregation_pipeline).to_list(None)

                # Convert results to UserSchema objects
                user_schema_list = []
                for result in results:
                    user_data = result["user"]
                    print("most_purchased_product ====>>> ", result["most_purchased_product"])
                    print("total_quantity ====>>> ", result["total_quantity"])
                    ...
                    print("most_purchased_product, total_quantity ====>>> ", most_purchased_product, total_quantity)
